...
from fxa._utils import APIClient, HawkTokenAuth, get_hmac, scope_matches
from fxa.cache import DEFAULT_CACHE_EXPIRY, MemoryCache
...
class Client:
    ...
    def trade_code(
        self, code, client_id=None, client_secret=None, code_verifier=None, ttl=None
    ):
    ...
    def authorize_code(
        ...
        body = {"client_id": client_id, "state": state}
        if scope is not None:
            body["scope"] = scope
        if code_challenge is not None:
    ...
    def verify_token(self, token, scope=None):
        ...
        key = "fxa.oauth.verify_token:%s:%s" % (
            get_hmac(token, TOKEN_HMAC_SECRET),
            scope,
        )
        ...
        if resp is None:
            ...
            missing_attrs = ", ".join(
                [k for k in ("user", "scope", "client_id") if resp.get(k) is None]
            )
            if missing_attrs:
