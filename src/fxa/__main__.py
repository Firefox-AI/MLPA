...
def main(args=None):
    ...
    parser.add_argument(
        "--bearer", help="Generate a Bearer token", dest="bearer", action="store_true"
    )
    parser.add_argument(
        "--unblock-account",
    ...
    parser.add_argument(
        "--auth", "-u", help="User credentials", dest="auth", required=False
    )
    parser.add_argument(
        "--out",
        "-o",
    ...
    parser.add_argument(
        "--verbose", "-v", help="Display status", dest="verbose", action="store_true"
    )

    # Creation args
    parser.add_argument(
        "--user-salt",
        help=("Salt used to calculate the user credentials. (Random by default)"),
        dest="fxa_user_salt",
        required=False,
    )
    ...
    if args["unblock_account"]:
        ...
        if len(auth) < 2:
            email = auth[0]
            password = getpass.getpass("Please enter a password for %s: " % auth[0])
    elif create:
        # Create a new user
