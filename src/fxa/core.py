...
from fxa._utils import APIClient, HawkTokenAuth, exactly_one_of, hexstr
...
class Client:
    ...
    def create_account(self, email, password=None, stretchpwd=None, **kwds):
        ...
        if self.key_stretch_version == 2:
            spwd = StretchedPassword(
                2, email, create_salt(2, hexlify(token_bytes(16))), password, stretchpwd
            )
            kb = token_bytes(32)
            body = {
                "email": email,
    ...
    def login(
        ...
        if self.key_stretch_version == 2:
            ...
            try:
                ...
                resp = self.finish_password_change_v2(password_change_token, spwd, kb)
                body = {
                    "email": email,
                ...
                print(
                    "Warning! v2 key stretch auto upgrade failed! Falling back to v1 login. "
                    + f"Reason: {inst}"
                )
                body = {
                    "email": email,
    ...
    def _get_stretched_password(self, email, password=None, stretchpwd=None):
        if password is not None:
            if stretchpwd is not None:
                raise ValueError(
                    "must specify exactly one of 'password' or 'stretchpwd'"
                )
    ...
    def destroy_account(self, email, password=None, stretchpwd=None):
        ...
        if isinstance(session.stretchpwd, bytes):
            ...
            isinstance(session.stretchpwd, StretchedPassword) and session.stretchpwd.v2
        ):
            stretchpwd = session.stretchpwd.v2
        elif (
            isinstance(session.stretchpwd, StretchedPassword) and session.stretchpwd.v1
        ):
        ...
        # destroy account
        url = "/account/destroy"
        body = {"email": email, "authPW": hexstr(derive_auth_pw(stretchpwd))}
    ...
    def change_password(
        self, email, oldpwd=None, newpwd=None, oldstretchpwd=None, newstretchpwd=None
    ):
        ...
        if self.key_stretch_version == 2:
            ...
            if version == 2:
                ...
                resp = self.start_password_change(email, old_spwd.v1)[
                    "passwordChangeToken"
                ]
                kb = self.fetch_keys(resp["keyFetchToken"], old_spwd.v1)[1]

            self.finish_password_change_v2(resp["passwordChangeToken"], new_spwd, kb)
        else:
            if oldpwd:
            ...
            self.finish_password_change(
                resp["passwordChangeToken"], newstretchpwd, new_wrapkb
            )
    ...
    def reject_unblock_code(self, uid, unblockCode):
        body = {"uid": uid, "unblockCode": unblockCode}
...
class Session:
    def __init__(
        ...
        if not isinstance(stretchpwd, StretchedPassword) and not isinstance(
            stretchpwd, bytes
        ):
            raise ValueError(
                "stretchpwd must be a bytes or a StretchedPassword instance, "
    ...
    def change_password(self, oldpwd, newpwd, oldstretchpwd=None, newstretchpwd=None):
        return self.client.change_password(
            self.email, oldpwd, newpwd, oldstretchpwd, newstretchpwd
        )
...
class PasswordForgotToken:
    def __init__(self, client, email, token, ttl=0, code_length=16, tries_remaining=1):
        self.client = client
        self.email = email
        self.token = token
        self.ttl = ttl
